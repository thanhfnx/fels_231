...
class DataValidator {
    ...
    class func validate(string: String?, fieldName: String, minimumLength: Int?,
        ...
        guard let string = string else {
            return (false, String(format: "EmptyFieldMessage".localized,
            fieldName))
        }
        if string.isEmpty {
            return (false, String(format: "EmptyFieldMessage".localized,
        ...
        if let min = minimumLength, string.characters.count < min {
            return (false,
            String(format: "InvalidLengthFieldMessage".localized,
            fieldName, min))
        }
        if let format = format {
            ...
            if !predicate.evaluate(with: string) {
                return (false,
                String(format: "InvalidFormatFieldMessage".localized,
                fieldName))
            }
        }
